...
def compute_lbp_uint8(gray):
    if gray.dtype != np.uint8:
        gray = gray.astype(np.uint8)
    # pad 边界为边缘复制
    p = np.pad(gray, ((1,1),(1,1)), mode='edge')
    center = p[1:-1,1:-1].astype(np.int32)
    neighbors = [
        p[0:-2,1:-1],   # top
        p[0:-2,2:],     # top-right
        p[1:-1,2:],     # right
    ...
    for i, nb in enumerate(neighbors):
        lbp |= ((nb.astype(np.int32) >= center) << (7 - i)).astype(np.uint8)
    return lbp
...
class BOP_Dataset(torch.utils.data.Dataset):
    def __init__(self, dataset_name, split, rank=0):
        self._DBG_MAX_VERBOSE = int(os.environ.get("MRC_DEBUG_VERBOSE_N", "3"))  # 只对前N个样本打印细节
        self._dbg_count = 0
        ...
        self.cache_dir = os.path.join(CUR_FILE_DIR, ".cache")
        ...
        for img_type in self.name_set:
            ...
            # ==== 调试输出：开始遍历子集 ====
            print(f"[数据加载] ◆开始遍历子集: '{img_type}' -> {train_dir}")
            ...
            # ==== 调试输出：CAD/对称信息加载完成 ====
            print(f"[数据加载] 已加载 CAD/对称信息: model_folder='{model_folder}'")
            ...
            for scene in sorted(os.listdir(train_dir)):  # scene
                ...
                scene_dir = os.path.join(train_dir, scene)
                 # ==== 调试输出：进入一个 scene 目录 ====
                print(f"[数据加载] ▶ 遍历 scene: {scene} (路径: {scene_dir})")
                ...
                scene_gt_bbox_dict = mmcv.load(os.path.join(scene_dir, "scene_gt_info.json"))  # gt_bboxes
                 # ==== 调试输出：三个 JSON 读取成功 ====
                try:
                    print(f"[数据加载] 已读取 JSON: scene_camera({len(scene_cam_dict)}), scene_gt({len(scene_gt_pose_dict)}),  scene_gt_info({len(scene_gt_bbox_dict)})")
                except Exception as _e:
                    print(f"[数据加载] JSON 统计打印失败: {type(_e).__name__}({_e})")

                  # 稀疏进度打印：每 scene 内每 500 张图像打印一次
                _dbg_img_idx = 0
                _dbg_every = 500
                ...
                for img_id_str in tqdm(scene_gt_pose_dict, postfix=f"{scene_id}"):  # image
                    ...
                    # ==== 调试输出：图像稀疏进度 ====
                    if (_dbg_img_idx % _dbg_every) == 0:
                        print(f"[数据加载] · 图像 {img_id_int:06d} 载入 OK，rgb='{os.path.basename(rgb_path)}'")
                    _dbg_img_idx += 1
            ...
            # ==== 调试输出：当前子集统计 ====
            try:
                print(f"[数据加载] ◆子集统计: {img_type}: images={image_counter}, instances={instance_counter}")
            except Exception as _e:
                print(f"[数据加载] 子集统计打印失败: {type(_e).__name__}({_e})")

        instance_level_dicts = []
        for img_dict in self.dataset_dicts:
            for inst in img_dict["annotations"]:
                inst_copy = inst.copy()
                inst_copy["obj_inst_count"] = img_dict["obj_inst_count"]
                inst_copy["img_type"] = img_dict["img_type"]
                instance_level_dicts.append(inst_copy)
        self.dataset_dicts = instance_level_dicts
        # ==== 调试输出：最终可用样本（instance-level） ====
        try:
            print(f"[数据加载] ◆最终可用样本: {len(self.dataset_dicts)} (instance-level)")
        except Exception as _e:
            print(f"[数据加载] 最终样本数打印失败: {type(_e).__name__}({_e})")
    ...
    def read_data(self, dataset_dict):
        assert 'obj_id' in dataset_dict, f"dataset_dict missing obj_id, keys: {dataset_dict.keys()}"
        dataset_dict = copy.deepcopy(dataset_dict)  # it will be modified by code below
        #inst_infos = dataset_dict.pop("inst_infos")
        inst_infos = dataset_dict  # 直接用本身
        ...
        if image.ndim == 2:
             ...
             # LBP 纹理
             lbp = compute_lbp_uint8(gray)
             image = np.stack([gray, edges, lbp], axis=-1)
        ...
        ## Dynamic Zoom-In (DZI) augmentation
        ...
        if self.split == 'train' or self.split == 'finetune':
            ...
            #dbgprint(f"[裁剪] 训练/微调 DZI: center={bbox_center.astype(np.float32)}, scale={float(bbox_scale):.3f}")
        else:
            ...
            #dbgprint(f"[裁剪] 测试 DZI(固定): center={bbox_center.astype(np.float32)}, scale={float(bbox_scale):.3f}")
        ...
        #dbgprint(f"[几何] fov(归一化中心与尺度): {fov.astype(np.float32)}")
        ...
        #dbgprint(f"[几何] roi_camK(3x3):\n{roi_camK.astype(np.float32)}")
    ...
    def _bg_img_paths(self):
        ...
        if osp.exists(cache_path):
            ...
            #print(f"[背景] 已从缓存读取: {cache_path} | 数量={len(bg_img_paths)}")
            return bg_img_paths
